...
def train(args):
    ...
    for epoch in range(int(args.reload_from)+1, config['nb_epoch']+1): 
        ...
        for batch in data_loader:
            ...
            #tree_batch, tree_node_num_batch, good_desc_batch, good_desc_len_batch, bad_desc_batch, bad_desc_len_batch = [tensor for tensor in batch]
            #loss = model(tree_batch, tree_node_num_batch.to(device), 
                        #good_desc_batch.to(device), good_desc_len_batch.to(device), bad_desc_batch.to(device), bad_desc_len_batch.to(device))
        ...
        # mark: pool_size need to change                 
        acc1, mrr, map1, ndcg = validate(valid_set, model, 3500, 1, config['sim_measure'])  
...
def parse_args():
    ...
    parser.add_argument('--model', type=str, default='ASTEmbeder', help='model name')
    parser.add_argument('--dataset', type=str, default='github12', help='name of dataset.java, python')
