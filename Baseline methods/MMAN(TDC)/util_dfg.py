...
def get_dfg_npy_info(lines, n_node, n_edge_types, state_dim, annotation_dim):
    ...
    for i in range(0, len(lines)):
        ...
        if (line[0:10] == 'BeginFunc:'): 
            ...
            dfg_info_list = lines[i+1].split() # node_num and edge_num of current dfg
            node_num, edge_num = int(dfg_info_list[0]), int(dfg_info_list[1])
...
def get_one_dfg_npy_info(lines, n_node, n_edge_types, state_dim):
    assert(lines[0][0:10] == 'BeginFunc:')
    
    dfg_info_list = lines[1].split() # node_num and edge_num of current dfg
    node_num, edge_num = int(dfg_info_list[0]), int(dfg_info_list[1])
...
def split_data(args):
    ...
    all_dfg_file_path = dir_path + args.all_dfg_file
    train_dfg_file_path = dir_path + args.train_dfg_file
    test_dfg_file_path = dir_path + args.test_dfg_file

    mark_list = []
    cnt = -1
    start_index = 0
    end_index = 0
    with open(all_dfg_file_path, 'r') as all_dfg_file:
        lines = all_dfg_file.readlines()
        for i in range(0, len(lines)):
            line = lines[i]
    ...
    print('all_num of dfg:\n', len(mark_list))

    with open(train_dfg_file_path, 'w') as train_dfg_file,  open(test_dfg_file_path, 'w') as test_dfg_file:
        for i in range(0, args.trainset_num):
            ind = index[i]
            for j in range(mark_list[ind][0], mark_list[ind][1]):
                train_dfg_file.write(lines[j])
        for i in range(args.testset_start_ind, args.testset_start_ind+args.testset_num):
            ind = index[i]
            for j in range(mark_list[ind][0], mark_list[ind][1]):
                test_dfg_file.write(lines[j])


def parse_args():
    parser = argparse.ArgumentParser("Prepare dfg data for dfgEmbeder")
    parser.add_argument('--data_path', type=str, default='./data/', help='location of the data corpus')
    parser.add_argument('--dataset', type=str, default='github11/', help='name of dataset.c')
    
    parser.add_argument('--all_dfg_file', type=str, default='all.dfg.txt')
    parser.add_argument('--train_dfg_file', type=str, default='train.dfg.txt')
    parser.add_argument('--test_dfg_file', type=str, default='test.dfg.txt')  
...
if __name__ == '__main__':
    ...
    with open(train_dfg_file_path, 'r') as train_dfg_file:
        lines = train_dfg_file.readlines()
        for i in range(0, len(lines)):
            line = lines[i]
            if line[0:10] == 'BeginFunc:' and i != 0:
                end_index = i
